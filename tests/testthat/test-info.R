...
test_that("info - Itempool", {
  ...
  object <- info(ip = ip, theta = theta)
  expect_identical(object, matrix(expected, nrow = 1), tolerance = 1e-6)
  # ---------------------------------------------------------------------------#
  theta = -0.017
  ...
  object <- info(ip = ip, theta = theta, tif = TRUE)
  expect_identical(object, matrix(expected, ncol = 1), tolerance = 1e-6)
  ...
  object <- info(ip = ip, theta = theta, tif = TRUE, observed = TRUE,
                 resp = resp)
  expect_identical(object, matrix(sum(expected), nrow = 1), tolerance = 1e-8)
...
test_that("info_response_set_cpp", {
  ...
  expect_identical(expected[1, 1], observed[i, 1])
