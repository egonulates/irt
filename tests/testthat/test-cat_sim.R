...
test_that("Test 'create_cat_design' function.", {
  ...
  expect_s3_class(cd, "cat_design")
  expect_identical(cd$title, title_text)
  ...
  true_ip <- itempool(data.frame(a = runif(n_ip, .5, 1.5), b = rnorm(n_ip)),
                      item_id = c(paste0(1:(n_ip - 1)), "last_item"))
  ...
  ip <- c(generate_ip(n = 5), itempool(sapply(1:length(temp_list$ids), function(i)
    generate_testlet(testlet_id = temp_list$ids[i], n = temp_list$n[i]))))
  ...
  theta <- rnorm(1)
  expect_s3_class(cd, "cat_design")
  co <- cat_sim(true_ability = theta, cd = cd)
  expect_s3_class(co, "cat_output")
  i <- 1
  expect_identical(co$est_history[[i]]$est_after,
                   co$est_history[[i+1]]$est_before)
  i <- 2
  expect_identical(co$est_history[[i]]$est_after,
                   co$est_history[[i+1]]$est_before)
  i <- 3
  ...
  cd <- create_cat_design(
    ...
    next_item_par = lapply(paste0("Item_", 1:20),
  ...
  # TODO: uncomment this and fix the issues in here:
  # cd <- create_cat_design(
  #   ip = ip,
  #   termination_rule = c('min_item', 'max_item'),
  ...
  #   termination_par = list(min_item = 8, max_item = 8),
  #   next_item_rule = 'fixed',
  #   next_item_par = list(item_id = c("Item_1", "testlet-8")))
  # expect_s3_class(cd, "cat_design")
  # expect_s3_class(cat_sim(true_ability = rnorm(1), cd = cd), "cat_output")
  #
  # cd <- create_cat_design(
  ...
  ### Ability Estimation Rule ###
  # expect_s3_class(create_cat_design(ip = ip, ability_est_rule = 'sum_score',
  ...
  #
  # ### Content Balancing ###
  # # expect_s3_class(create_cat_design(
...
test_that("Test cat_sim Function", {
  ...
  true_ability <- rnorm(1)
  expected <- cat_sim(true_ability = true_ability, cd = cd)
  expect_s3_class(expected, "cat_output")
  expect_type(expected$true_ability, "list")
  expect_identical(unlist(expected$true_ability), true_ability)
  expect_identical(length(expected$est_history), 5L)
  ...
  expect_identical(length(expected), length(true_ability))

  # -------------------------------------------------------------------------- #
  # if true_theta has names, the CAT output will be also named
  ...
  cd <- create_cat_design(ip = ip, next_item_rule = 'mepv',
                          termination_rule = 'max_item',
                          termination_par = list(max_item = 5))
  true_ability <- setNames(rnorm(5), sample(letters, 5))
  observed <- cat_sim(true_ability = true_ability, cd = cd)
  expect_equal(names(observed), names(true_ability))
  i <- sample(1:5, 1)
  expect_equal(observed[[i]]$examinee_id, names(true_ability)[i])

  # -------------------------------------------------------------------------- #
  # By default examinees named as S1, S2, ...
  n <- sample(1:10, 1)
  observed <- cat_sim(true_ability = rnorm(n), cd = cd)
  i <- sample(1:n, 1)
  expect_equal(names(observed)[i], paste0("S", i))
  expect_equal(observed[[i]]$examinee_id, paste0("S", i))


  # -------------------------------------------------------------------------- #
  # A separate cat_design for each examinee can be provided.
  ip <- generate_ip(n = 20)
  ...
  cd3 <- create_cat_design(ip = ip,
                           termination_rule = c('max_item'),
                           termination_par = list(max_item = 3))

  cd <- c(cd1, cd2, cd3)
  co <- cat_sim(true_ability = rnorm(3), cd = cd)
  expect_equal(co[[1]]$test_length, 5)
  expect_equal(co[[2]]$test_length, 9)
  # Enforce the lengths of cat_design and true ability are the same
  expect_error(cat_sim(true_ability = rnorm(1), cd = cd),
               "Invalid 'cd'. When a list of 'cat_design' objects provided")


  # -------------------------------------------------------------------------- #
  # Function raises an error if the true_theta is not numeric
  ip <- generate_ip(n = 20)
  cd <- create_cat_design(ip = ip,
                          termination_rule = c('max_item'),
                          termination_par = list(max_item = 5))
  expect_error(cat_sim(true_ability = c("Ally", "Wally"), cd = cd),
               "'true_ability' should be a numeric vector")
  expect_error(cat_sim(true_ability = c(-1, 2, "ABC"), cd = cd),
               "'true_ability' should be a numeric vector")
  expect_error(cat_sim(true_ability = list(c(1, 2), c(3, "ABC"), 5), cd = cd),
               "'true_ability' should be a numeric vector")
  expect_error(cat_sim(true_ability = c(1, NA), cd = cd),
               "'true_ability' should not contain any missing")
  expect_error(cat_sim(true_ability = list(c(1, 2), c(3, NA), 5), cd = cd),
               "'true_ability' should not contain any missing")
  expect_error(cat_sim(true_ability = list(c(1, 2), c(3, -2.4),
                                           as.numeric(NA)), cd = cd),
               "'true_ability' should not contain any missing")

  expect_error(cat_sim_fast(true_ability = c("Ally", "Wally"), cd = cd),
               "'true_ability' should be a numeric vector")
  expect_error(cat_sim_fast(true_ability = c(-1, 2, "ABC"), cd = cd),
               "'true_ability' should be a numeric vector")
  expect_error(cat_sim_fast(true_ability = list(c(1, 2), c(3, "ABC"), 5), cd = cd),
               "'true_ability' should be a numeric vector")
  expect_error(cat_sim_fast(true_ability = c(1, NA), cd = cd),
               "'true_ability' should not contain any missing")
  expect_error(cat_sim_fast(true_ability = list(c(1, 2), c(3, NA), 5), cd = cd),
               "'true_ability' should not contain any missing")
  expect_error(cat_sim_fast(true_ability = list(c(1, 2), c(3, -2.4),
                                                as.numeric(NA)), cd = cd),
               "'true_ability' should not contain any missing")
  # -------------------------------------------------------------------------- #
  # -------------------------------------------------------------------------- #
  ...
  # Note: the following test sometimes fails, If it does again, check it.
  expect_identical(
    unname(est_ability(resp = resp[-max_item], ip = administered_ip[-max_item],
                       method = "ml")$est),
    co$est_history[[max_item]]$est_before,
    tolerance = 0.001)
  # Final ability estimate calculated correctly:
  expect_identical(
    unname(est_ability(resp = resp, ip = administered_ip, method = "ml")$est),
    co$est_history[[max_item]]$est_after,
    tolerance = 0.001)
  ...
  for (i in seq_len(n_testlet)) {
    ...
    ip_list <- c(ip_list, testlet(itempool(b = rnorm(temp),
                                       item_id = paste0("t", i, "-i", 1:temp)),
                          testlet_id = paste0("t", i)))
  }
  ...
  expect_s3_class(test, 'cat_output')
  expect_identical(length(test$est_history), 20L)
  ...
  for (i in seq_len(n_testlet)) {
    ...
    ip_list <- c(ip_list, testlet(itempool(
      b = rnorm(temp), item_id = paste0("t", i, "-i", 1:temp)),
      testlet_id = paste0("t", i)))
  }
  # Add individual items
  ip <- c(itempool(ip_list), itempool(b = rnorm(n_testlet)))
  ...
  test <- cat_sim(true_ability = rnorm(1), cd = cd)
  expect_s3_class(test, 'cat_output')
  expect_identical(length(test$est_history), 30L)
  # An intermediate ability estimate is correctly calculated
  k <- sample(2:28, 1)
  expect_identical(as.numeric(sum(sapply(test$est_history, `[[`, "resp")[1:k])),
                   test$est_history[[k]]$est_after)
  # Final ability estimate is correctly calculated
  expect_identical(as.numeric(sum(sapply(test$est_history, `[[`, "resp"))),
  ...
  test = cat_sim(true_ability = rnorm(1), cd = cd)
  expect_identical(test$est_history[[1]]$item$item_id, 'i3')
  expect_identical(test$est_history[[2]]$item$item_id, 'i2')
  expect_identical(test$est_history[[3]]$item$item_id, 'i4')
  expect_identical(test$est_history[[4]]$item$item_id, 'i5')
  expect_identical(test$est_history[[5]]$item$item_id, 'i1')
  # summary(test)
  ...
  expect_identical(length(test), 3L)


  # TODO: UNCOMMENT THIS and FIX THESE ISSUES
  # # -------------------------------------------------------------------------- #
  ...
  # expect_s3_class(test, 'cat_output')
  # eh <- test$est_history
  # expect_identical(eh[[1]]$item, ip["Item_1"][[1]])
  # expect_identical(eh[[2]]$testlet, ip["testlet-8"][[1]])
  # expect_identical(eh[[2]]$item, ip["testlet-8"][[1]]$item_list[[1]])
  ...
  expect_s3_class(test, 'cat_output')
  n <- sample(1:20, 1)
  expect_identical(test$est_history[[n]]$item, ip[[n]])
  ...
  # The ability estimate just before the administration of the last item is
  # indeed calculated correctly:
  est_r <- est_ability(ip = administered_ip[-max_item], resp = resp[-max_item],
                       method = "ml", theta_range = c(min_theta, max_theta))$est
  ...
  expect_identical(unname(est_r), est_cpp, tolerance = 0.001)
  # Final ability estimate calculated correctly:
  expect_identical(
    est_ability(resp = resp, ip = administered_ip, method = "eap",
                theta_range = c(min_theta, max_theta))$est,
    co$est_history[[max_item]]$est_after, tolerance = 0.001)
  ...
  for (i in seq_len(n_testlet)) {
    ...
    ip_list <- c(ip_list, testlet(itempool(
      b = rnorm(temp), item_id = paste0("t", i, "-i", 1:temp)),
      testlet_id = paste0("t", i), misc = list(
  ...
  n_theta <- 3L # Number of examinees
  ...
  expect_identical(length(expected), n_theta)
  ...
  ip <- generate_ip(n = n_ip,
                    misc = lapply(c(0, rep(1, n_ip)),
                                  function(k) list("sympson_hetter_k" = k)))
  ip[[1]]$a <- 3 # Make the item very eligible
  cd <- create_cat_design(ip = ip,
                          termination_rule = c('max_item'),
                          termination_par = list(max_item = 3),
                          next_item_rule = 'mepv',
  ...
  expect_s3_class(cd, "cat_design")
  expect_s3_class(co <- cat_sim(true_ability = rnorm(1), cd = cd), "cat_output")
  expect_false("Item_1" %in% sapply(co$est_history, function(k) k$item$item_id))
  expect_true("Item_2" %in% sapply(co$est_history, function(k) k$item$item_id))
  ...
  test <- cat_sim(true_ability = rnorm(1), cd = cd)
  expect_s3_class(test, 'cat_output')
  expect_identical(length(test$est_history), 5L)
  ...
  resp <- get_cat_response_data(co, output_type = "matrix")
...
#   expect_identical(item$item_id,  ip[[13]]$item_id)
...
#   expect_true(selected_item$item_id %in% ip[-1]$item_id)
#
#
#   # Fixed test
#   n_ip = 5
...
#   est_history = list("0" = list(est = 0.2, se = 0.3,  resp = 0, item = ip[[1]]),
#                      "1" = list(est = .4, se = .2,  resp = 1, item = NULL))
#   expect_identical(select_next_item_cpp(cd = cd, est_history = est_history),
#                     ip[[2]])
# })
...
#   n <- 5 # number of items
#   ip <- itempool(data.frame(b = c(20, 20, 20, 20, 20)), item_id = paste0("i", 1:n))
#   # CAT Design
#   cd = create_cat_design(ip = ip, next_item_rule = 'random',
#                        termination_rule = 'max_item',
#                        termination_par = list(max_item = 3))
...
#   expect_identical(generate_cat_response(cd = cd, est_history = est_history,
#                                      true_ability = 0), 0)
#
#   true_ip = itempool(data.frame(b = c(-20, -20, -20, -20, -20)),
#                       item_id = paste0("i", 1:n))
...
#     expect_identical(x1$est, x2$est)
#     expect_identical(x1$se, x2$se)
#   }
#
#   # ------------------------------------------------------------------------ #
...
#   expect_identical(ability_est_cat(cd = cd, est_history = est_history)$est, 1)
#   est_history$`2` = list(est = NA, se = NA, item = ip[[2]], resp = 0)
#   expect_identical(ability_est_cat(cd = cd, est_history = est_history)$est, 1)
#   est_history$`3` = list(est = NA, se = NA, item = ip[[3]], resp = 1)
#   expect_identical(ability_est_cat(cd = cd, est_history = est_history)$est, 2)
