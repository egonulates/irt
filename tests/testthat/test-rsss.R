...
test_that("rsss", {
  ...
  raw_score <- sort(sample(0:sum(ip$item_max_score), n_score))
  ...
  expect_identical(raw_score[i], sum(mean(ip, theta = scale_score[i])),
                   tolerance = tol)
  ...
  expect_identical(raw_score[i], expected, tolerance = tol)
  ...
  expected <- sum(p * matrix(0:(ncol(p) - 1), nrow = nrow(p), ncol = ncol(p),
                             byrow = TRUE), na.rm = TRUE)
  expect_identical(rsss(ip = ip, scale_score = scale_score,
