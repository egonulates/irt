...
############################################################################@###
test_that("get_remaining_items_cpp", {
  n_ip <- 12
  ...
  remaining_ip <- irt:::get_remaining_items_cpp(cd, est_history, list())
  expect_s4_class(remaining_ip, "Itempool")
  expect_true(validObject(remaining_ip))
  remaining_ids <- remaining_ip$item_id
  selected_ids <- ip[selected_items]$item_id
  expect_false(any(selected_ids %in% remaining_ids))

  # -------------------------------------------------------------------------- #
  ### Testlet ###
  t1 <- testlet(itempool(b = -3:-2, item_id = c("t1-i1", "t1-i2")), testlet_id = "t1")
  ...
  expect_true(validObject(remaining_ip))
  remaining_ids <- remaining_ip$item_id
  expect_true("t1" %in% remaining_ip$testlet_id)
  expect_false("t2" %in% remaining_ip$testlet_id)
  ...
  expect_false("t1" %in% remaining_ip$testlet_id)
  # expect_true("t2" %in% remaining_ids)
  expect_true("t2" %in% remaining_ip$testlet_id)
  ...
  remaining_ip <- irt:::get_remaining_items_cpp(cd, est_history, additional_args)
  expect_s4_class(remaining_ip, "Itempool")
  expect_true(validObject(remaining_ip))
  remaining_ids <- remaining_ip$item_id
  expect_false("t1" %in% remaining_ids)
...
test_that("Test get_administered_items_cpp Function", {
  ...
  expect_s4_class(administered_ip, "Itempool")
  # expect_true(validObject(administered_ip))
  expect_identical(length(administered_ip@item_list), 0L)
  ...
  expect_s4_class(administered_ip, "Itempool")
  expect_true(validObject(administered_ip))
  expect_identical(length(administered_ip), 1L)
  expect_true(ip[[3]]$item_id %in% administered_ip$item_id)
  expect_false(ip[[1]]$item_id %in% administered_ip$item_id)
  expect_false(ip[[2]]$item_id %in% administered_ip$item_id)
  expect_false(ip[[4]]$item_id %in% administered_ip$item_id)
  expect_false(ip[[5]]$item_id %in% administered_ip$item_id)
  ...
  expect_s4_class(administered_ip, "Itempool")
  expect_true(validObject(administered_ip))
  expect_identical(length(administered_ip), 2L)
  expect_true(ip[[3]]$item_id %in% administered_ip$item_id)
  expect_true(ip[[2]]$item_id %in% administered_ip$item_id)
  expect_false(ip[[1]]$item_id %in% administered_ip$item_id)
  expect_false(ip[[4]]$item_id %in% administered_ip$item_id)
  expect_false(ip[[5]]$item_id %in% administered_ip$item_id)
  ...
  expect_s4_class(administered_ip, "Itempool")
  expect_true(validObject(administered_ip))
  expect_identical(length(administered_ip), 3L)
  expect_true(ip[[3]]$item_id %in% administered_ip$item_id)
  expect_true(ip[[1]]$item_id %in% administered_ip$item_id)
  expect_false(ip[[2]]$item_id %in% administered_ip$item_id)
  expect_false(ip[[4]]$item_id %in% administered_ip$item_id)
  expect_true(ip[[5]]$item_id %in% administered_ip$item_id)
})
...
############################################################################@###
test_that("Test loglik_est_history_cpp Function", {
  ...
  expect_identical(irt:::loglik_est_history_cpp(est_history = est_history,
                                        theta = theta, calculate_loglik = TRUE),
               resp_loglik(ip = ip[[1]], resp = resp[1], theta = theta))
  expect_identical(irt:::loglik_est_history_cpp(
  ...
  expect_error(
    irt:::loglik_est_history_cpp(est_history = est_history, theta = theta),
...
test_that("Test est_ability_cat_cpp Function", {
  ...
  expected <- est_ability(ip = ip, resp = resp, method = "eap")
  observed <- est_ability_cat_cpp(true_ability = rnorm(1), cd = cd,
                                  est_history = est_history,
                                  additional_args = list())
  observed <- observed$est_history
  observed <- observed[[length(observed)]]
  expect_identical(expected$est, observed$est_after,  tolerance = 1e-5)
  expect_identical(expected$se, observed$se_after, tolerance = 1e-5)
  ...
  expected <- est_ability(resp = resp, ip = ip, method = "owen",
                          prior_pars = c(0, 2))
  ...
  expect_identical(expected$est, observed$est, tolerance = 1e-8)
  expect_identical(expected$se, observed$se, tolerance = 1e-8)
  ...
  expect_identical(observed$est_history[[4]]$est_after, as.numeric(sum(resp)))
  ...
  expected <- est_ability(ip = ip, resp = resp, method = "map")
  observed <- est_ability_cat_cpp(true_ability = rnorm(1), cd = cd,
                                  est_history = est_history,
                                  additional_args = list())
  observed <- observed$est_history
  observed <- observed[[length(observed)]]
  expect_equal(expected$est, observed$est_after, tolerance = 1e-5)
  expect_equal(expected$se, observed$se_after, tolerance = 1e-5)


  # -------------------------------------------------------------------------- #
  # "map_ml" -- imperfect response
  n_ip <- 3
  ip <- itempool(a = runif(n_ip, .5, 1.5), b = rnorm(n_ip))
  cd <- create_cat_design(ip = ip, next_item_rule = 'random',
                          ability_est_rule = "map_ml",
                          ability_est_par = list(prior_dist = "norm",
                                                 prior_par = c(0, 1),
                                                 min_theta = -5,
                                                 max_theta = 5,
  ...
  expected <- est_ability(ip = ip, resp = resp, method = "ml")
  observed <- est_ability_cat_cpp(true_ability = rnorm(1), cd = cd,
                                  est_history = est_history,
                                  additional_args = list())
  observed <- observed$est_history
  observed <- observed[[length(observed)]]
  expect_equal(unname(expected$est), observed$est_after,  tolerance = 1e-5)
  expect_equal(unname(expected$se), observed$se_after, tolerance = 1e-5)


  # -------------------------------------------------------------------------- #
  # "map_ml" -- perfect response
  n_ip <- 3
  ip <- itempool(a = runif(n_ip, .5, 1.5), b = rnorm(n_ip))
  cd <- create_cat_design(ip = ip, next_item_rule = 'random',
                          ability_est_rule = "map_ml",
...
test_that("Test est_ability_4pm_nr Function", {
  ...
  for (i in 1:10) {
    ...
    expect_identical(mlnr, mlr$est, tolerance = 0.001, ignore_attr = TRUE)
  }
  ...
  for (i in 0:1) {
    ...
    expect_identical(est, theta_range[i + 1])
  }
  ...
  expect_identical(unname(est_opt), est_nr, tolerance = 1e-3)
  ...
  #   resp = resp, ip = ip, theta_range = c(-5, 5), criterion = 1e-06,
  #   initial_estimates = c(-5 + 1e-06, 0, 5 - 1e-06))
  # expect_identical(est_opt, est_nr, tolerance = 1e-3)
  #
...
test_that("terminate_test_cpp Function", {
  ...
  cd <- create_cat_design(
    ...
    next_item_rule = "fixed",
    next_item_par = list(item_id = x$item_id),
  ...
  cd <- create_cat_design(
    ip = ip,
    next_item_rule = "fixed",
    next_item_par = list(item_id = ip$item_id),
...
test_that("cat_sim_single_cpp Function", {
  ...
  expect_s3_class(cs, "cat_output")
  expect_identical(n_ip, length(cs$est_history))
  ...
  # expect_identical(true_theta, mean(expected$est_ability), tolerance = 0.3)
  #
  #
  #
  # ## Micro benchmarking ##
  ...
  #     testlet_id = paste0("t", i), misc = list(sympson_hetter_k = runif(1, .5, 1))))
  # }
...
test_that("cat_sim_single_cpp Function", {
  t1 <- testlet(itempool(b = -4:-2, item_id = c("t1-i1", "t1-i2", "t1-i3")), testlet_id = "t1")
  t2 <- testlet(itempool(b = 2:3, item_id = c("t2-i1", "t2-i2")), testlet_id = "t2")
  ip <- c(t1, t2, itempool(b = -1:4))
  observed <- irt:::get_itempool_size(ip)
  expect_identical(observed[1], setNames(8L, "elements"))
  expect_identical(observed[2], setNames(2L, "testlets"))
  expect_identical(observed[3], setNames(11L, "items"))
})
